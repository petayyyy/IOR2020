    ...
    def callback(self,data):                                                                                         # Основная функция (data- изображения из типа msg)
        if self.Color == True or self.Qr == True:
            ...
            if self.Qr == True:
                ...
                try:
                    self.image_pub_potato.publish(self.bridge.cv2_to_imgmsg(res1, "bgr8"))                                      # Вывод в отдельный топик
                except CvBridgeError as e:
                    print(e)

                res2 = cv2.bitwise_and(img, img, mask= mask2)                                                                # И так с каждым цветом
                try:
                    self.image_pub_water.publish(self.bridge.cv2_to_imgmsg(res2, "bgr8"))
                except CvBridgeError as e:
                    print(e)

                res3 = cv2.bitwise_and(img, img, mask= mask3)
                try:
                    self.image_pub_seed.publish(self.bridge.cv2_to_imgmsg(res3, "bgr8"))
                except CvBridgeError as e:
                    print(e)

                res4 = cv2.bitwise_and(img, img, mask= mask4)                                                                # И так с каждым цветом
                try:
                    self.image_pub_pastures.publish(self.bridge.cv2_to_imgmsg(res4, "bgr8"))
                except CvBridgeError as e:
                    print(e)

                res5 = cv2.bitwise_and(img, img, mask= mask5)
                try:
                    self.image_pub_soil.publish(self.bridge.cv2_to_imgmsg(res5, "bgr8"))
                except CvBridgeError as e:
                    print(e)
                ...
                for c in potato:                                                                                                # Перебор каждого контура
                    try:
                        ...
                        if sum_pixel > 20:
                            ...
                            self.ploh[self.Pole].append(['Potato',start.x+x_d,start.y+y_d])
                    ...
                    try:
                        ...
                        if sum_pixel > 20:
                            ...
                            if len(approx) < 10:
                                ...
                                self.ploh[self.Pole].append(['Water',start.x+x_d,start.y+y_d])
                                cv2.drawContours(img, [c], 0, (0, 0, 0), 2)
                            else:
                    ...
                    try:
                        ...
                        if sum_pixel > 20:
                            ...
                            if len(approx) < 10:
                                ...
                                self.ploh[self.Pole].append(['Seed',start.x+x_d,start.y+y_d])
                                cv2.drawContours(img, [c], 0, (0, 0, 0), 2)
                            else:
                    ...
                    try:
                        ...
                        if sum_pixel > 20:
                            ...
                            if len(approx) < 10:
                                ...
                                self.ploh[self.Pole].append(['Pastures',start.x+x_d,start.y+y_d])
                                cv2.drawContours(img, [c], 0, (0, 0, 0), 2)
                            else:
                ...
                for c in soil:
                    try:
                        ...
                        if sum_pixel > 20:
                            ...
                            self.ploh[self.Pole].append(['Soil',start.x+x_d,start.y+y_d])
                            cv2.drawContours(img, [c], 0, (0, 0, 0), 2)
                    except:pass
...
print navigate(x=3, y=1, z=1.5, speed=0.5, yaw=math.radians(90),frame_id='aruco_map')
col_det.Pole = 'D'
col_det.Color = True
rospy.sleep(10)
col_det.Color = False

print navigate(x=3, y=3, z=1.5, speed=0.5, yaw=math.radians(90),frame_id='aruco_map')
col_det.Pole = 'B'
col_det.Color = True
rospy.sleep(10)
col_det.Color = False
